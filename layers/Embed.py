...
def apply_rotary_pos_emb(x, cos, sin, offset: int = 0):
    cos, sin = (
        cos[offset : x.shape[0] + offset, ...],
        sin[offset : x.shape[0] + offset, ...],
    )
...
class ListPatchEmbedding(nn.Module):
    def __init__(
        ...
        self.position_embedding = PositionalEmbedding(d_model=d_model)
        # self.RotaryEmbedding=RotaryEmbedding(d_model=d_model)
        # self.SinusoidalPositionalEmbedding=SinusoidalPositionalEmbedding(d_model=d_model)
        self.dropout = nn.Dropout(dropout)
        self.augmentation = nn.ModuleList(
            [get_augmentation(aug) for aug in augmentation]
        )
        # self.eRPE_embedding=eRPE(emb_size=16,num_heads=8,seq_len=5000,dropout=0.1)
    ...
    def forward(self, x):  # (batch_size, seq_len, enc_in)
        ...
        # x1 = x.permute(0, 1, 2)[0]  # Just permuting dimensions for heatmap

        # # Move the tensor to CPU for plotting
        # x1_cpu = x1.detach().cpu().numpy()  # Ensure it's on CPU before converting to numpy

        # # Plotting the heatmap
        # plt.figure(figsize=(10, 8))
        # sns.heatmap(x1_cpu, cmap='viridis')
        # plt.xlabel('Position')
        ...
        # save_path = '/home/zzz/pycharm_project_49/image'
        # os.makedirs(save_path, exist_ok=True)  # Ensure the directory exists
        # plt.savefig(os.path.join(save_path, 'x.png'))
        # plt.close()
        # print("x saved at:", os.path.join(save_path, 'x.png'))
        ...
        for padding, value_embedding in zip(self.paddings, self.value_embeddings):
            ...
            x_list.append(x_new)
        # x = x.to('cuda')
        # x1 = x.permute(0, 1, 2).to('cuda')
        # pe_values = self.position_embedding(x1)[0].cpu().numpy()  # Shape: (max_len, d_model)

        # # Plotting the heatmap
        # plt.figure(figsize=(10, 8))
        # sns.heatmap(pe_values, cmap='viridis')
        # plt.xlabel('Position')
        # plt.ylabel('Embedding Dimension')
        ...
        # plt.close()
        # print("Heatmap saved at:", os.path.join(save_path, 'heatmap.png'))
        ...
        # cos,sin=self.RotaryEmbedding(x)

        # x=[
        #     apply_rotary_pos_emb_torch(x,cos,sin,cxt)
        #     for x,cxt in zip(x_list,self.learnable_embeddings)
        # ]
