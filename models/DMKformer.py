...
from layers.Mambaformer_EncDec import Encoder, EncoderLayer
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == "classification":
            ...
            self.kan=KAN([dim,configs.num_class,dim])
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        # print("enc_out",enc_out[0].shape)

        enc_out, attns = self.encoder(enc_out, attn_mask=None)
        if self.single_channel:
            enc_out = torch.reshape(enc_out, (-1, self.enc_in, *enc_out.shape[-2:]))

        layer_norm = torch.nn.LayerNorm(enc_out.size(-1)).to(enc_out.device)
        enc_out = layer_norm(enc_out)  # 对 enc_out 进行层归一化
        enc_out = self.kan(enc_out)
